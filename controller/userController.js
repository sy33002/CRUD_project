...
exports.postLogin = async (req, res) => {
  ...
  try{
    ...
    if (result != null) {
      ...
      res.status(500).send({ result: false, message: '서버 오류가 발생했습니다.' });
  }
}
...
exports.postSignup = async (req, res) => {
  try {
    const { userId, userPw, userName, userAddr, userEmail, userCategory } = req.body;
    ...
    res.status(500).send({ result: false, message: '오류가 발생했습니다.' });
  }
};
